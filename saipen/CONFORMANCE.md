...
1. **Repo Validation**: `STATE.md`, `BOARD.md`, and `LOG.md` MUST conform to the shapes RFC.md § 1.2 defines, enforced today by `tests/validate.sh` / `validate.ps1`. `extensions/schemas/*.json` are a forward-looking reference for a future external validator, not machine-enforced yet -- see `extensions/schemas/README.md`.
