...
2. **Determine the real translation surface -- cover everything real, fabricate nothing:**
   ...
   - **Docs**: translate what's covered by (a) above that doesn't already have a hand-maintained per-language sibling. Never re-translate or overwrite a file the project already maintains by hand per language (check for an existing `<name>_XX.md`-style file first, e.g. this repo's own `guides/`) -- note it as already covered instead of duplicating or clobbering curated work.
