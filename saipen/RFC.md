...
- The `extensions/security/`, `extensions/performance/`, and `extensions/multi-agent/` folders inside the SAIPEN home itself are reference *examples* for a project author to copy and adapt -- the same role `extensions/templates/` plays for `.saipen/` boilerplate. They are not live hooks the SAIPEN home reads on its own behalf; nothing discovers them unless a project author has replicated the pattern into their own project root. `multi-agent/` differs from the other two in shape, not in kind: it doesn't attach to a single phase, it changes who may write `STATE.md`/`BOARD.md`/`LOG.md` for the whole session -- still project-attached, still opt-in, still never read by SAIPEN's own home on its own behalf.
