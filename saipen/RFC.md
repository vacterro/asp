...
- Agent MUST NOT write secrets (API keys, tokens, passwords, private credentials) into `STATE.md`, `BOARD.md`, `LOG.md`, or `KNOWLEDGE/` -- these files are meant to be committed and read by other agents. A secret discovered mid-task gets redacted in whatever gets written (`sk-***`, not the real value) and the user gets told to rotate it, same as any other finding.
