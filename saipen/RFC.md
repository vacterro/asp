...
- `saipen stop` -- MUST checkpoint immediately per § 1.5 (flush `LOG.md`, update `BOARD.md`, set an explicit resumable `next_action`), then halt and return control to the user. MUST NOT leave a ticket mid-edit. Overrides `goal_mode` -- it is the user's manual brake and always wins. **This is a pause, not an exit**: `goal_mode` (if `true`) is left exactly as-is -- § 2.4's Exit list is exhaustive and `stop` is not on it. Any later resume, whether the user types `saipen continue`, bare `saipen`, or bare `saipen goal`, continues under that same still-`true` `goal_mode` precisely as if the stop had never happened; bare `saipen goal`'s resume clause (§ 2.4 Entry) is one legal way to resume, never the only one.
