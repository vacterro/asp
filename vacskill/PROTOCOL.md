# vacskill Protocol v4

Vendor-neutral project execution protocol for LLM agents. Memory owns the
project; the model is a temporary worker. Agents read the same `.vacskill/`,
continue, write back. Chat history is not memory. This file is the whole
protocol -- load it in one breath; STYLE.md (voice) loads with it, UI.md only
for UI work. Style unreachable? Say so once, work facts-only.
...
## Capabilities -- the protocol degrades, not the agent

On resume/init note what you actually have; missing pieces switch rules:

| Missing | Degradation |
|---|---|
| git | no SHIP/PUBLISH; backups via `.vacskill/history/`; hunt anchor = file mtimes |
| terminal/exec | VERIFY = `MANUAL-VERIFY` only, conf never above low |
| file write | read-only advisor: report findings, write nothing, say so first |
| network | no PUBLISH, no dependency audits; note it once in LOG |

Never fake a capability. A degraded run that says so beats a pretty lie.
...
redirects (PowerShell `>`, `Out-File`). Found UTF-16 -> convert, LOG a DEC.
Legacy `.vac/` -> rename (`git mv` when tracked, plain move otherwise), LOG a
DEC, never run both. No secrets -- travels with the repo.
...
`.vacskill/history/NAME-DD.MM.YY-HHmm.md`, deleting all but the newest 5 of
that name in the same breath.
...
**Init** (no `.vacskill/`) -- copy `templates/` (STATE, BOARD, LOG; KNOWLEDGE/,
tmp/ on first need) or create equivalent. Ensure root `AGENTS.md` has the
block below (search `VACSKILL:BEGIN`, never duplicate). `VACSKILL_HOME` =
absolute folder of THIS file. `CLAUDE.md`/`GEMINI.md` missing -> create each:
...
- **P2 reliability** -- silent catches, missing timeouts, leaks, unbounded
  growth. **P3** -- duplication 3+, dead code, missing tests.
...
user's own remote, never anyone else's.
...
2. Version, smallest step: micro -> letter `3.1.0a` (docs repos only); little
   change -> `3.2.1`; feature batch -> `3.2.0`; breaking -> major, rare.
3. Before `git add`: .gitignore covers junk + secrets (node_modules,
   __pycache__, dist, .env*, .vacskill/tmp|history/) -- missing -> write it.
   Empty `tmp/`, strip debug prints.
4. CHANGELOG.md newest-top, 1-2 lines per version = the commit message. Push.
5. `git tag -a vVERSION -m "CHANGELOG LINE"` + push tags. Tags are the
   archive; no `_archive_*` copies.
6. First publish (no origin): confirm name + public/private with user;
   afterwards ship without asking.
...
rule evicts a stale one; the protocol stays loadable in one breath.
...
9. Never fake a capability; degrade loudly per the table above.
10. STYLE voice holds to the last response. Corporate prose = drift.